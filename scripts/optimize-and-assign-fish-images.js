...
// Check if source images are in Image/Fish or already in public/Image/Fish
const sourceDir = path.join(__dirname, '../Image/Fish');
const publicDir = path.join(__dirname, '../public/Image/Fish');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Fish/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Fish/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'FISH' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} FISH products with optimized images`);
