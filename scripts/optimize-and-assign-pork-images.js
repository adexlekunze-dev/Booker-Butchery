...
// Check if source images are in Image/Pork or already in public/Image/Pork
const sourceDir = path.join(__dirname, '../Image/Pork');
const publicDir = path.join(__dirname, '../public/Image/Pork');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Pork/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Pork/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'PORK' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} PORK products with optimized images`);
