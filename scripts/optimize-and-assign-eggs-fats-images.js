...
// Check if source images are in Image/Eggs & fats or already in public/Image/Eggs & fats
// Note: Folder is "Eggs & fats" but category is "EGGS & FATS"
const sourceDir = path.join(__dirname, '../Image/Eggs & fats');
const publicDir = path.join(__dirname, '../public/Image/Eggs & fats');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Eggs & fats/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Eggs & fats/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'EGGS & FATS' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} EGGS & FATS products with optimized images`);
