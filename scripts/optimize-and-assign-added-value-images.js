...
// Check if source images are in Image/Added value or already in public/Image/Added value
// Note: Folder is "Added value" but category is "ADDED VALUE"
const sourceDir = path.join(__dirname, '../Image/Added value');
const publicDir = path.join(__dirname, '../public/Image/Added value');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Added value/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Added value/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'ADDED VALUE' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} ADDED VALUE products with optimized images`);
