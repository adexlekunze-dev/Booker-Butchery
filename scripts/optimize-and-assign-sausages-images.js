...
// Check if source images are in Image/Sausage or already in public/Image/Sausage
// Note: Folder is "Sausage" but category is "SAUSAGES"
const sourceDir = path.join(__dirname, '../Image/Sausage');
const publicDir = path.join(__dirname, '../public/Image/Sausage');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        // Note: Folder is "Sausage" but we'll use "Sausage" in the path
        optimizedPaths.push(`/Image/Sausage/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Sausage/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'SAUSAGES' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} SAUSAGES products with optimized images`);
