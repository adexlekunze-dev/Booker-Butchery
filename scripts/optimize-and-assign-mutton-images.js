...
// Check if source images are in Image/Mutton or already in public/Image/Mutton
const sourceDir = path.join(__dirname, '../Image/Mutton');
const publicDir = path.join(__dirname, '../public/Image/Mutton');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Mutton/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Mutton/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'MUTTON' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} MUTTON products with optimized images`);
