...
// Check if source images are in Image/Veal or already in public/Image/Veal
const sourceDir = path.join(__dirname, '../Image/Veal');
const publicDir = path.join(__dirname, '../public/Image/Veal');
...
// Image files - handle both "Image 1.png" (with space) and "Image1.png" (no space)
const imageFiles = ['Image 1.png', 'Image 2.png', 'Image 3.png', 'Image 4.png'];
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Veal/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Veal/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'VEAL' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} VEAL products with optimized images`);
