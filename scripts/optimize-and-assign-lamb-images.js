...
// Check if source images are in Image/Lamb or already in public/Image/Lamb
const sourceDir = path.join(__dirname, '../Image/Lamb');
const publicDir = path.join(__dirname, '../public/Image/Lamb');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Lamb/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Lamb/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'LAMB' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} LAMB products with optimized images`);
