...
// Check if source images are in Image/Poultry & game or already in public/Image/Poultry & game
const sourceDir = path.join(__dirname, '../Image/Poultry & game');
const publicDir = path.join(__dirname, '../public/Image/Poultry & game');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        // Note: Use the actual folder name (Next.js will handle URL encoding)
        optimizedPaths.push(`/Image/Poultry & game/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Poultry & game/${normalizedName}`);
      }
    } else {
      // No sharp: just copy (Next.js will optimize on-demand)
      fs.copyFileSync(sourcePath, publicPath);
      optimizedPaths.push(`/Image/Poultry%20%26%20game/${normalizedName}`);
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'POULTRY & GAME' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} POULTRY & GAME products with optimized images`);
