...
// Check if source images are in Image/Chicken or already in public/Image/Chicken
const sourceDir = path.join(__dirname, '../Image/Chicken');
const publicDir = path.join(__dirname, '../public/Image/Chicken');
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (sharp) {
      try {
        ...
        optimizedPaths.push(`/Image/Chicken/${normalizedName.replace('.png', '.webp')}`);
      } catch (error) {
        console.log(`❌ Error optimizing ${fileName}:`, error.message);
        // Fallback: just copy the file
        fs.copyFileSync(sourcePath, publicPath);
        optimizedPaths.push(`/Image/Chicken/${normalizedName}`);
      }
    } else {
...
async function updateProducts() {
  ...
  products.forEach(product => {
    if (product.category === 'CHICKEN' && product.active) {
  ...
  console.log(`\n✅ Updated ${updatedCount} CHICKEN products with optimized images`);
