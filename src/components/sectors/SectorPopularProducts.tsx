...
type SectorPopularProductsProps = {
  sectorName: string;
  sectorSlug: string;
  socialProof?: {
    count: number;
...
export function SectorPopularProducts({
  sectorName,
  sectorSlug,
  socialProof,
}: SectorPopularProductsProps) {
  ...
  const scrollContainer = (direction: "left" | "right") => {
    const container = document.getElementById("sector-popular-products-carousel");
    if (!container) return;
    ...
    <section className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-start justify-between mb-8">
          ...
          <Link 
            href={`/sectors/${sectorSlug}/shop?best_seller=true`}
        ...
        <div className="relative">
          ...
          <div
            id="sector-popular-products-carousel"
          ...
          >
            {displayProducts.length > 0 ? (
              ...
              <div className="w-full text-center py-12 text-gray-500">
                <p>Loading popular products for {sectorName}...</p>
              </div>
            )}
          </div>
          {showRightButton && (
